...
Maybe increase the number of rows and columns, or even change the
shape entirely? Figure~\ref{fig:as-graph} (a) gives an example of the
magic pentagram game, in which the product of the labels (\(+1\) or
\(-1\)) assigned to each of four vertices in a line must equal the
labels of that line (the product of the four vertices in the topmost
horizontal line, for example, must be \(-1\)). Notice that we are now
changing the visualization format to
hypergraphs. Figure~\ref{fig:as-graph} (b) shows the magic square as a
hypergraph. In this section, we explore a characterization of games that
follow the same style as the magic square and pentagram, given
by~\ref{arkhipov2012}.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Examples of hypergraphs representing quantum magic games~\ref{arkhipov2012}.}
\end{figure}
\printbibliography{}
\end{document}
